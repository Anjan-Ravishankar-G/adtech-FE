...
export default function Sidebar({ selectedTab, setSelectedTab }: SidebarProps) {
  ...
  return (
    <div className="flex">
      ...
      >
        {isOpen ? <X size={32} className="bg-[#D1D5DB] text-black rounded-2xl p-1" /> : <Menu size={32} />}
      </button>
      {/* Sidebar and Content Wrapper */}
      <div className={`h-screen flex transition-all duration-300 ${isOpen ? "ml-64" : "ml-1"}`}>
        {/* Sidebar */}
        <div
          className={`h-screen bg-[#D1D5DB] text-white transition-all duration-300 ${
            ...
            <ul className="space-y-4 mt-16">
              <li>
                <button
                  ...
                  className={`w-full text-left py-2 px-4 rounded text-black ${
